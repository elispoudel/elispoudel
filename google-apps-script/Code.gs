...
function servePdf(fileId) {
  ...
  const output = ContentService.createBlobOutput(file.getBlob()).setMimeType(ContentService.MimeType.PDF);
...
function parseBody_(e) {
  ...
  if (typeof contents === 'string') {
    try {
      ...
      // Ignore and try form-style parsing below.
    }

    if (contents.indexOf('payload=') === 0 || contents.indexOf('payload%3D') === 0) {
...
function respond_(e, obj, status) {
  if (e && e.parameter && e.parameter.response === 'postMessage') {
