...
class LoginManager {
  ...
  async handleLogin(event) {
    ...
    if (typeof FirebaseService !== "undefined" && FirebaseService.isEnabled()) {
      try {
        const { role } = await FirebaseService.signIn(username, password);
        this.showMessage("Login successful. Redirecting...", "success");
        setTimeout(() => {
          window.location.href = role === "admin" ? APP_PATHS.adminPanel : APP_PATHS.dashboard;
        }, 800);
        return;
      } catch (err) {
        const message =
...
class DashboardManager {
  ...
  async init() {
    if (typeof FirebaseService !== "undefined" && FirebaseService.isEnabled()) {
      const hasSession = await FirebaseService.restoreSession();
      if (!hasSession) {
    ...
    // If admin, setup edit functionality
...
class DownloadManager {
  ...
  downloadCV() {
    try {
      ...
      const cvPath = window.location.pathname.includes('/pages/') ? '../assets/documents/elis-cv.pdf' : 'assets/documents/elis-cv.pdf';
      link.href = cvPath;
    ...
    if (buttons && buttons.length > 0) {
      buttons.forEach((btn) => {
        btn.addEventListener("click", (e) => {
          ...
          if (cls) {
            window.location.href = APP_PATHS.classDetail(cls);
          }
        });
      });
    }
