...
(function () {
  ...
  function loadEmbeddedPreview(url, fileId) {
    const canvasWrap = modal.querySelector('.pdf-canvas-wrap');
    ...
    iframe.className = 'pdf-embedded-frame';
    iframe.title = document.getElementById('pdfTitleBar')?.textContent || 'PDF document';
    iframe.loading = 'eager';
    iframe.allow = 'autoplay';
    iframe.src = fileId ? getDrivePreviewUrl(fileId) : url;
    iframe.addEventListener('load', () => showLoader(false), { once: true });
    iframe.addEventListener('error', () => {
      showLoader(false);
      showError('Failed to load the document. Make sure the Google Drive file is shared publicly.');
    }, { once: true });
  ...
  async function renderPage(num) {
    ...
    try {
      ...
      const maxWidth = Math.max(320, (canvasWrap?.clientWidth || viewport.width) - 32);
  ...
  async function openViewer({ url, fileId, title, containerId } = {}) {
    errorEl.style.display = 'none';
    if (!url && fileId) {
    ...
    if (containerId) {
      embeddedContainer = document.getElementById(containerId);
      if (embeddedContainer) {
        embeddedContainer.innerHTML = '';
        embeddedContainer.appendChild(modal);
    ...
    // set title
    try {
      const titleText = title || (url && url.split('/').pop()) || (fileId ? `File ${fileId}` : 'PDF Document');
    ...
    showModal();
    if (containerId) {
      loadEmbeddedPreview(url, fileId);
      return;
    }
